...
export type MutationMutateLikeArgs = {
  id: Scalars['ID'];
};
...
export type MutateLikeMutationVariables = Exact<{
  id: Scalars['ID'];
}>;

export type MutateLikeMutation = { __typename?: 'Mutation' } & {
  mutateLike: { __typename?: 'Post' } & Pick<Post, 'id'> & {
      likes?: Maybe<Array<{ __typename?: 'User' } & Pick<User, 'id'>>>;
      author: { __typename?: 'User' } & Pick<User, 'id'>;
    };
};
...
export type GetPostByIdQueryResult = Apollo.QueryResult<
  ...
  GetPostByIdQueryVariables
>;
export const MutateLikeDocument = gql`
  mutation MutateLike($id: ID!) {
    mutateLike(id: $id) {
      id
      likes {
        id
...
`;
export type MutateLikeMutationFn = Apollo.MutationFunction<
  MutateLikeMutation,
  MutateLikeMutationVariables
>;

/**
 * __useMutateLikeMutation__
 *
 * To run a mutation, you first call `useMutateLikeMutation` within a React component and pass it any options that fit your needs.
 * When your component renders, `useMutateLikeMutation` returns a tuple that includes:
 * - A mutate function that you can call at any time to execute the mutation
 * - An object with fields that represent the current status of the mutation's execution
 *
 * @param baseOptions options that will be passed into the mutation, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options-2;
 *
 * @example
 * const [mutateLikeMutation, { data, loading, error }] = useMutateLikeMutation({
 *   variables: {
 *      id: // value for 'id'
 *   },
 * });
 */
export function useMutateLikeMutation(
  baseOptions?: Apollo.MutationHookOptions<
    MutateLikeMutation,
    MutateLikeMutationVariables
  >,
  ...
  const options = { ...defaultOptions, ...baseOptions };
  return Apollo.useMutation<MutateLikeMutation, MutateLikeMutationVariables>(
    MutateLikeDocument,
    options,
  );
}
export type MutateLikeMutationHookResult = ReturnType<
  typeof useMutateLikeMutation
>;
export type MutateLikeMutationResult = Apollo.MutationResult<MutateLikeMutation>;
export type MutateLikeMutationOptions = Apollo.BaseMutationOptions<
  MutateLikeMutation,
  MutateLikeMutationVariables
>;
